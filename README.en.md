...
- [x] **In-app one-click update**: manual "Check now" in settings, banner "Update now" → download (live progress) → replace in place → restart; falls back to browser download
- [x] **Readable connection-failure reasons**: auth failure / timeout / refused / unreachable / DNS / host-key mismatch, each with next-step guidance
- [x] SFTP download resume (cancelled/failed transfers keep the partial file and auto-resume)
- [x] Double-click word / triple-click line selection; find navigation with Enter / Shift+Enter
- [x] Immersive wallpapers (built-in Minimal Light / Dark, custom image upload) with theme colors derived from the wallpaper
- [x] Platform font stacks: UI uses platform default fonts (macOS SF Pro/PingFang, Windows Segoe UI/YaHei, Linux Ubuntu/Cantarell/Noto); terminal defaults to JetBrains Mono (with an ExtraLight variant for dim text)
...
## How Rudder Is Developed

Rudder is developed collaboratively by its maintainer and an AI coding assistant ([WorkBuddy](https://www.workbuddy.cn), powered by GLM):

- **Maintainer-driven**: product direction, architecture decisions, code review, on-device verification, and releases
- **AI-implemented**: feature work, bug fixes, tests, CI/CD maintenance, and docs — all changes are generated by the AI from the maintainer's requirements
- Every change is verified by local builds, the GitHub Actions 6-platform build matrix, and manual review before release
