...
- **修复：打开「进程」或「系统信息」窗口只显示 macOS 交通灯、内容不出现（拖动窗口边缘拉伸一下才出来）。** 根因：**macOS 上新映射的第二个窗口不会自动产生首次渲染事件**，而 Slint 的布局是"渲染时惰性计算"的 —— 没渲染过就等于没算过布局，于是那个窗口一直是空的，直到外部原因（手动拖动边缘）送来一次 `Resized` 才补上首帧。现在在 `show()` 之后按 0 / 50 / 200ms 各**显式请求一次重绘**（`Window::request_redraw()`，Slint 为此提供的 API）；首次请求可能早于窗口映射完成而被 winit 丢弃，所以补两次。**不改尺寸** —— 那只是把"拖一下边缘"自动化。顺带把系统信息窗口的尺寸改用 Slint 自己的 `set_size`（原先用 winit 的 `request_inner_size` 在 Slint 背后动 OS 窗口），并删掉两个窗口 ScrollView 里虚高的 `viewport-height`（它会形成"视口更大 → 内容被拉更长"的正反馈）。**Fixed: the Processes / System information window showed only the macOS traffic lights with no content** (until the window was manually resized). Root cause: macOS never delivers a first-render event for a newly mapped second window, and Slint computes layout lazily at render time — so the window was never laid out or painted until an external `Resized` (the manual drag) arrived. The app now explicitly requests a redraw via `Window::request_redraw()` at 0 / 50 / 200 ms after `show()` (the first request can be dropped by winit before the window is mapped, hence the two follow-ups). No size change is involved. Also: the system-info window now sizes itself through Slint's own `set_size` instead of winit's `request_inner_size` behind Slint's back, and both windows drop the inflated `viewport-height` binding that created a "bigger viewport → taller content" feedback loop.
...
- **修复：外观页还原后「界面字体」显示为「内嵌字体」。** 出厂默认是空串 = auto，实际生效值是平台相关的**字体栈**（如 `SF Pro Text, …, Heiti SC, Meatshell Mono`）。此前把 auto 硬映射到下标 0，而 0 是 `▍内嵌字体` 这条**不可选择的分组标题**，于是下拉框把标题当成了当前字体。现在索引换算与启动时共用一套规则：按栈里第一个真实存在的家族定位，且回退时只回退到可选家族、永不回退到标题。**Fixed: after restoring Appearance defaults the "UI font" field showed "Embedded fonts".** The factory default is empty = auto, whose effective value is a platform-specific font *stack*. Auto was hard-mapped to index 0 — which is the non-selectable `▍Embedded fonts` group header — so the combo box showed a header as the current font. Index resolution now shares one rule with startup: it picks the first family in the stack that actually exists, and never falls back to a header.
- **修复：布局页还原后运行状态侧栏跑到了右侧。** `LayoutSettings::default()` 长期是 `"right"`，而 `ui/app.slint` 声明的 `sidebar-dock` 是 `"left"` —— 播种与还原都取 Rust 这一侧，于是"还原"把侧栏从左挪到右。现在出厂默认统一为 `left`，并新增测试把 Rust 默认值与 Slint 声明值钉在一起（空白停靠边的老配置同样落到左侧）。**Fixed: restoring Layout defaults moved the status sidebar to the right.** `LayoutSettings::default()` had long been `"right"` while `ui/app.slint` declares `sidebar-dock: "left"`; both seeding and restore read the Rust side, so restoring moved the panel. The factory default is now `left`, with a test pinning the Rust default to the Slint declaration (legacy configs with an empty dock also land on the left).

- **修复：「还原本页默认」还原到的不是出厂默认值。** 各页还原读的是 `ConfigFile::default()`（派生 Default：全 0 / 全 false / 全空串），而不是真正的出厂默认 —— 结果终端字体退回 Meatshell Mono、字体大小显示 0、回滚行数被钳到 100、LF→CRLF 与 OSC52 被关闭、光标类型变回方块、内置规则集显示空白。现在新增页还原统一以 `fresh_config()`（与全新安装相同的出厂默认）为唯一数据源，并显式写全终端页各项：字体 JetBrains Mono / 13px / 光标竖线 / 光标颜色 #FFFFFF / 回滚 5000 行。**Fixed: "restore this page" did not restore factory defaults.** The reset handlers read `ConfigFile::default()` — the derived Default (all zeros / false / empty), not the factory defaults — so the terminal font fell back to Meatshell Mono, the size showed 0, scrollback got clamped to 100, LF→CRLF and OSC52 turned off, and the cursor reverted to a block. Page resets now share `fresh_config()` with brand-new installs and explicitly write every terminal-page item (JetBrains Mono / 13px / bar cursor / #FFFFFF / 5000 lines).
- **修复：布局页还原导致面板相互重叠。** 布局是派生状态：`sidebar_dock` / `welcome_as_sidebar` 变更之后，停靠边冲突消解、面板几何量与窗格树都必须重算，此前只写了 5 个属性。现在把启动时的布局播种抽成 `apply_layout_prefs` 供启动与还原共用，还原时持久化后延迟一帧执行整套迁移（含 welcome 页在侧栏与窗格树之间的迁移与 `refresh_panes`），并补上此前缺失的持久化。**Fixed: resetting the Layout page produced overlapping panels.** Layout is derived state — after `sidebar_dock` / `welcome_as_sidebar` change, the dock-conflict resolution, per-panel geometry and the pane tree must be recomputed. The reset only wrote five properties. The startup layout seeding is now extracted as `apply_layout_prefs` and reused, the transition is deferred by one frame (avoiding #323), and the change is persisted.
- **修复：快捷命令栏入口会彻底消失。** 命令栏的「Quick」入口按钮被绑定到 `quick-panel-open`，而不是"快捷命令是否停靠为侧栏"——布局还原把 `quick-commands-as-sidebar` 关掉之后侧栏消失、入口按钮却仍被隐藏，快捷命令就此无法访问。现按 #215 的原意改为 `quick-commands-as-sidebar && quick-panel-open`。**Fixed: the quick-command bar entry could disappear entirely.** The bar's "Quick" button was bound to `quick-panel-open` instead of "are quick commands docked as a sidebar" — turning the sidebar off left the button hidden, making quick commands unreachable. It now follows #215's intent: `quick-commands-as-sidebar && quick-panel-open`.
- **修复：设置 → 外观 → 「动画」开关没有后端，重启即回。** 此前它只写 Slint 全局（`AnimationSettings.enabled`），配置里没有对应字段。现在新增 `animations_disabled` 持久化字段，启动时由 config 播种、切换时写盘，「还原本页默认」也会把它恢复为开启。**Fixed: the Interface › Animations toggle had no backend and reset on restart.** It only wrote the Slint global (`AnimationSettings.enabled`) with no config field behind it. There is now a persisted `animations_disabled` field, seeded from config at startup, saved on toggle, and restored to enabled by the appearance page's reset.
- **修复：「隐藏特殊分区」归错了还原页。** 它的控件在 UI 上位于「外观」页，还原逻辑却把它挂在「传输」页 —— 还原外观不会重置它，还原传输反而会动它。现移到外观页的还原里。**Fixed: "Hide special partitions" was reset by the wrong page.** Its control lives on the Appearance page, but the reset logic had it under Transfer — resetting Appearance left it alone while resetting Transfer changed it. It now belongs to the Appearance reset.
- **移除：「新版本提示」页的「还原本页默认」按钮。** 该页只有启动检查一个开关，没有需要还原的内容。**Removed: the "reset this page" button on the Update page.** That page only holds the startup-check switch — nothing worth resetting.

### 变更 / Changed

- **出厂默认：壁纸遮罩透明度 15% → 30%。** 仅迁移仍停留在旧默认（15%）的用户，自定义滑条值一律保留；`defaults_rev` 升至 5。**Factory default: wallpaper overlay transparency 15% → 30%.** Only users still sitting on the old 15% default are migrated; custom slider values are preserved (`defaults_rev` bumped to 5).
...
- **设置页「还原本页默认」。** 终端 / 外观 / 布局 / 传输 / 更新五页各有一个「还原本页默认」按钮，位于该页最后一个设置项之后、随内容滚动，两段式确认（还原 → 确认还原？）后把该页设置恢复为出厂默认。只还原设置项（A 类）；壁纸与遮罩、挂载点过滤、自定义高亮规则等一律保留——高亮规则只逐条停用，不删除数据。WSL 与同步页全部是用户数据与账户配置，不提供还原。**"Reset this page" in Settings.** Terminal / Appearance / Layout / Transfer / Update each gain a reset button placed after that page's last setting (it scrolls with the content); a two-step confirmation restores that page to the factory default. Only settings (category A) are reset — wallpaper and overlay, mount-point filters and custom highlight rules are preserved (rules are disabled one by one, never deleted). WSL and Sync are entirely user data and account config, so they offer no reset.
### 变更 / Changed

- **出厂默认值更新四项（仅影响新用户）。** 默认字体族显式指定为内置的 JetBrains Mono；粘贴时默认转换行尾；输出高亮预设默认为 Rudder 内置规则；新用户的欢迎页不再默认常驻侧栏。`DEFAULTS_REV` 未提升，因此已有用户的 `sessions.json` 值保持不变。**Four factory defaults changed (new users only).** The default font family is now the embedded JetBrains Mono; pasting converts line endings by default; the output-highlight preset defaults to Rudder's built-in rules; and the welcome page no longer starts docked as a sidebar. `DEFAULTS_REV` was not bumped, so existing `sessions.json` values are untouched.
- **设置页导航重排**为：终端 → 外观 → 传输 → 布局 → [WSL] → 同步 → 更新（传输前移至布局之前）。**Settings navigation reordered** to Terminal → Appearance → Transfer → Layout → [WSL] → Sync → Update.
- **About 对话框视觉优化**：淡入淡出、卡片上浮、版本徽章、依赖列表排版（版本数据来源不变）。**About dialog polish**: fade in/out, card lift, version badge and dependency list typography (the version data source is unchanged).
- **移除标签拖拽分屏 / 合并（drag-to-split）。** 拖动标签不再显示 drop-zone 高亮框，也不再触发分屏或合并；标签条内的左右换序、点击选中不受影响，分屏仍可通过标签右键菜单的「向右分屏 / 向下分屏」完成，后续会另行重构实现。**Tab drag-to-split / merge was removed.** Dragging a tab no longer shows the drop-zone highlight or triggers a split or merge; reordering within the strip and click-to-select are unaffected, and splitting remains available from the tab context menu (Split right / Split down). A fresh implementation is planned.
- **移除标签双击「复制连接」手势（#340）。** 重命名只保留右键菜单一条路径，双击不再触发任何行为。**The tab double-click "duplicate connection" gesture was removed (#340).** Renaming is available from the context menu only; double-click no longer does anything.
- **侧栏拖拽的性能改动（live/applied 宽度拆分 + 松手缩放动画）经尝试后已回退**，净效果为零，侧栏拖拽行为与 0.7.5 一致。**A sidebar-drag performance change (splitting a live width from the applied one, plus a release animation) was tried and reverted** the same day; the net effect is zero and sidebar dragging behaves exactly as in 0.7.5.
- **「信任自签名/内网证书」的说明改为明确写出后果**："将不再校验服务器证书，连接可能被中间人冒充"。此前只说"仅用于可信 NAS 或局域网"，没有点明它是关闭身份校验。**The "Trust self-signed / intranet certs" description now states the consequence**: "stops verifying the server certificate — the connection can be impersonated". It previously only said "trusted NAS or LAN", without spelling out that identity verification is off.

### 安全 / Security

- **expect/send 触发器的「应答」此前以明文写入 `sessions.json`。** 该字段的文档一直写着"与登录密码一样加密存储"，但保存时并未处理它——而它的典型用法正是自动回答 `Password:` 提示。现在与登录密码使用同一套加密，旧配置在下次保存时自动迁移。**Trigger responses were stored in plaintext in `sessions.json`.** The field has always documented itself as "stored encrypted like the session password", but the save path never touched it — and the usual way to fill it in is answering a `Password:` prompt. It now shares the password's encryption, and existing configs migrate on the next save.
- **`https://` 代理不再被静默当作明文 HTTP CONNECT 处理。** 这条路径会把 `Proxy-Authorization: Basic`（base64 明文凭据）写在未加密的连接上。现在会明确报错并提示改用 `socks5://` 或 `http://`——注意会话对话框本身只生成这两种，所以受影响的只有手改过的配置和 `ALL_PROXY` 环境变量。**An `https://` proxy is no longer silently treated as a plaintext HTTP CONNECT.** That path put `Proxy-Authorization: Basic` (base64 credentials) on an unencrypted connection. It now fails with the two working alternatives; note the session dialog only ever emits the other two schemes, so this only affects hand-edited configs and `ALL_PROXY`.
- **`secret.key` 长度异常时不再就地覆盖。** 旧文件会改名保留为 `secret.key.bad`，并记录一条 error 级日志；同时加载时若发现"仍是密文却解不开"的密码会统计并报错，而不是让那串 `enc:v1:` 被当成密码发出去。**A wrong-sized `secret.key` is no longer overwritten in place.** It is kept as `secret.key.bad` with an error log, and load now counts passwords that remain unreadable instead of silently passing the stale `enc:v1:` blob to the server.

### 内部 / Internal

- **`run()` 从 2070 行降到 803 行：设置播种与持久化回调整体下沉。** 原本夹在「进程监视窗」与「wire 装配」之间的 1264 行（主题、字体、壁纸、快捷命令、UI 缩放、WebDAV、各页「还原本页默认」等）移入新的 `app/settings_ui.rs::seed_settings`，`src/app.rs` 由 3632 行降到 2303 行。调用次序保持不变，为纯重构，无用户可见行为变化。**`run()` shrank from 2070 to 803 lines: settings seeding moved out.** The 1264 lines that sat between the process-monitor window and the wiring step (theme, fonts, wallpaper, quick commands, UI scale, WebDAV, per-page "reset" …) moved into a new `app/settings_ui.rs::seed_settings`; `src/app.rs` went from 3632 to 2303 lines. Ordering is unchanged — a pure refactor with no user-visible behaviour change.
- **`AppContext` 成为唯一的状态根集合。** 五个 `wire_*` 函数与 `seed_settings` 现在都接收 `&AppContext`，`SessionWireCtx`（21 字段）与 `TabWireCtx`（13 字段）已删除 —— 此前正是「手工维护的字段副本漏字段」导致了关闭标签时泄漏 `tab_statuses`。**`AppContext` is now the single set of state roots.** All five `wire_*` functions plus `seed_settings` take `&AppContext`; the hand-maintained `SessionWireCtx` (21 fields) and `TabWireCtx` (13 fields) are gone — a missing field in one of those copies is exactly what caused the closed-tab `tab_statuses` leak.
- **配置拆成两个文件：`settings.json`（设置）与 `sessions.json`（会话）。** 此前每改一个开关都要把含加密口令的全部会话重新序列化、重新加密再整体重写一遍。现在两者同目录、共享 `secret.key`，改设置不再碰会话文件。**老配置零迁移**：合并前的 `sessions.json` 仍可原样读入，首次启动时自动把设置那半边另存成 `settings.json`（有 `settings.json` 时以它为准）。**The config is split in two: `settings.json` (settings) and `sessions.json` (sessions).** Previously every toggle rewrote the whole file, re-encrypting every stored password. They now live side by side and share `secret.key`. **No migration for existing installs**: a pre-split `sessions.json` still loads as-is, and the settings half is written out to `settings.json` on the first launch (which takes precedence from then on).
- **写盘防抖：连续改动合并成一次落盘（250 ms）。** 缩放 / 面板字号 / 终端字号是逐帧触发的滑条，此前一次拖动会产生几十次全量写盘；命令历史也是每执行一条就写一次。现在这些路径只标记「待写」，由定时器收尾，退出前强制 `flush`。**Debounced config writes (250 ms).** Scale, panel-font and terminal-font sliders fire on every frame, so a single drag used to trigger dozens of full rewrites, and command history was written on every command. Those paths now only mark the store dirty; a trailing timer performs the write, with a forced flush on exit.

### 性能 / Performance

- **资源采样不再每秒重枚举挂载点（实测 -89% 主线程开销）。** 用 samply 采样发现 1 Hz 的 `SystemSampler` 每次 tick 要花约 28 ms，其中 92% 来自 `Disks::refresh`——它会对每个挂载点执行 `statfs`。改为每 10 次 tick 才重枚举一次（CPU / 内存 / 网络仍每秒刷新），每次 tick 降到约 3 ms，主线程墙钟占用从 2.82% 降到约 0.4%，同时消除了每秒一次约 28 ms 的周期性主线程停顿。**The resource sampler no longer re-enumerates mounts every second (measured: −89%).** Profiling with samply showed the 1 Hz `SystemSampler` costing ~28 ms per tick, 92% of it in `Disks::refresh` (a `statfs` per mount point). It now re-enumerates only every 10th tick — CPU / memory / network still refresh every second — bringing a tick down to ~3 ms and main-thread wall occupancy from 2.82% to ~0.4%, and removing a ~28 ms main-thread stall every second.

### 修复 / Fixed

- **双击自绘标题栏无法最大化（三平台）。** 标题栏在 pointer-down 时就调用 `win-drag()` 把指针交给系统，第一次点击的 up 不再派发，`double-clicked` 因此永远不满足。改为位移阈值上膛：无位移的双击正常最大化 / 还原，超过 6px 的拖动照常移动窗口。**Double-clicking the custom title bar did not maximize (all three platforms).** The title bar called `win-drag()` on pointer-down, handing the pointer to the system so the first click's up was never dispatched and `double-clicked` could never fire. It now arms on a 6px movement threshold: a double-click without movement maximizes/restores, a longer drag moves the window as before.
- **macOS：交通灯右侧的 28px 让位条双击无反应。** 该区域为不干扰系统窗口拖拽而没有命中区，成了死区——既不在原生标题栏范围内，也没有自己的处理。现补上只处理双击的命中区（**不加** `win-drag`，拖拽继续交给系统），双击即可最大化。**macOS: the 28px strip right of the traffic lights ignored double-clicks.** It deliberately had no hit area so as not to interfere with system window dragging, which left a dead zone. It now handles double-click only (without `win-drag`, so dragging stays with the system).
- **重命名标签后标签条仍显示旧名。** `refresh_panes` 复用窗格旧标签子模型的条件只比较标签 id，而重命名改的是 title，于是被判定为"未变化"继续复用含旧标题的模型。现在比较加入 title：标题变化时派生子模型，纯几何刷新仍复用旧模型，不 churn 标签条。**Renaming a tab left the old title in the strip.** The condition for reusing a pane's previous tab sub-model compared only the tab id, while renaming changes the title. The comparison now includes the title: a title change derives a new sub-model, while purely geometric refreshes still reuse the old one.
- **设置页内容结束后留有一大片滚动空白。** `viewport-height` 原为 `max(可见高度, 内容高度)`，内容不足一屏时被强行撑满；改为严格等于内容高度，有多少设置就滚多少。**Settings left a large empty scroll area past the last item.** `viewport-height` was `max(visible, content)`, forcing a full screen even when the content was shorter; it now equals the content height exactly.
- **关闭标签页会泄漏该标签的资源状态。** `on_pane_tab_closed` 清理了终端缓冲、渲染闸门、SFTP 句柄等六类资源，唯独漏掉 `tab_statuses`（进程列表 / 网络历史 / 磁盘行），而清理函数拿不到它——因为它不在 `TabWireCtx` 里。反复开关标签会持续累积。**Closing a tab leaked its status entry.** The close handler released six kinds of per-tab resources but not `tab_statuses` (process list / network history / disk rows) — the handler could not even reach it, because it was missing from `TabWireCtx`.
- **两处真实文件驱动的终端回归测试此前一直在静默跳过。** 它们读取 `../terminal_chars_test.txt`，而该文件位于 `tests/` 下、cargo 运行测试时的工作目录是包根目录，路径永远解析不到，于是直接 `return`。现改为 `include_bytes!` 编译期嵌入，与 PPK fixture 的做法一致。**Two real-file terminal regression tests were silently skipping.** They read `../terminal_chars_test.txt`, but the file lives under `tests/` while cargo runs tests from the package root, so the read never resolved and the test returned early. They are now embedded at compile time via `include_bytes!`, matching the PPK fixtures.
- **SOCKS5 握手不再无视客户端提供的方法列表。** 之前无论对方是否提供「无认证」都一律回 `05 00`；现在若未提供则按协议回 `05 FF`（no acceptable methods）。**The SOCKS5 handshake no longer ignores the client's method list.** It always answered `05 00` regardless; it now replies `05 FF` when "no authentication" was not offered.
- **收敛了生产路径上的 6 处 `unwrap`/`expect`。** 分布在不同文件里、且同一函数内其它同类调用已经做了降级处理（锁中毒 → 复用数据 / 跳过本次），属于不一致而非设计。现在全仓 `lock().unwrap()` 为 0 处。**Six production-path `unwrap`/`expect` calls were brought in line with their neighbours.** Each sat next to similar calls that already degraded gracefully (poisoned lock → reuse the data / skip this pass); the outliers were inconsistencies, not design. The tree now has zero `lock().unwrap()`.
- **`-D` 动态转发绑定到非本机地址时会在终端输出一条警告**，说明该端口无认证、同网段设备均可借道。**A `-D` forward bound beyond loopback now prints a warning** into the terminal explaining that the port has no authentication and is usable by anything on that network.
...
- **53 号色块回归 / Cube-53 regression**: 修复 SGR 拦截器把 `48;5;53m` 的 256 色索引 `53` 误判为 SGR 53(上划线)并剥离的问题——被误伤后 cell 背景变为透明,53 号色块显示为主题背景色(纯黑/纯白背景下即黑/白);同样误伤 `38;5;53`(前景)与 `48;5;21`/`38;5;21`(被误重写为双下划线)。拦截器现跳过 `38;5;N`/`48;5;N` 的索引参数,独立 `53m` 上划线不受影响
- **256 色标准色恢复 / Exact xterm palette**: 移除暗色主题对 256 色背景的提亮逻辑,所有色块(含 53 号暗品红)恢复精确 xterm 标准值,与 Windows Terminal / PowerShell 渲染一致;提亮曾导致 53 号与 91 号视觉重合

### 新特性 / Features

- **系统字体可选 / System fonts in picker**: 设置 → 终端字体下拉框新增系统等宽字体(fontdb 枚举),按「内嵌字体 / 外置字体 / 系统字体」分组标题展示,系统字体列在最后;选择后仅保存纯家族名,配置可移植
- **分组标题列表 / Grouped font picker**: 字体下拉框从逐条带前缀改为分组标题(▍内嵌字体 / ▍外置字体 / ▍系统字体)+ 缩进家族名,标题行不可选

### 清理 / Cleanup

- 修复 3 个编译警告:窗口关闭回调重命名(`close` → `request-close`,避免遮蔽内置函数)、无用 `mut`、死代码标注

### 测试 / Tests

- 回归测试 169 项全部通过,新增:拦截器不误伤 256 色索引、render() 完整路径 53 号精确品红、完整测试文件端到端 53 号精确值、暗色主题精确 xterm 色板
