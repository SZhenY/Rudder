...
pub(crate) struct AppContext {
    ...
    /// Saved-session list backing the welcome page and the quick-connect search.
    pub(crate) sessions_model: Rc<VecModel<SessionInfo>>,
    /// Latest local-machine sample + its network history sparkline.
    pub(crate) local_snap: LocalSnap,
    pub(crate) local_net_hist: NetHist,
    /// Live "SFTP follows the terminal's cd" flag; the shell event pumps read it
    /// on every CwdChanged, so toggling it applies to open sessions too.
...
impl AppContext {
    pub(crate) fn new(config: ConfigStore, window: AppWindow) -> anyhow::Result<Self> {
        ...
        // Saved-session list. Seeded here rather than later in `run()` so every
        // consumer of the context sees a populated model.
        let sessions_model: Rc<VecModel<SessionInfo>> = Rc::new(VecModel::default());
        window.set_sessions(ModelRc::from(sessions_model.clone()));
        sync_sessions_for_window(&window.as_weak(), &store.borrow(), &sessions_model);
...
pub fn run() -> Result<()> {
    ...
    // Windows / Linux 的"自动"：探测一次（子进程真渲染一帧，约一秒），把结论**固化进配置** ——
    // 有 GPU 就存 `wgpu`（FemtoVG on wgpu），没有就存 `software`；之后每次启动直接读配置，
    // 不再探测。想重新探测（换机器 / 装了显卡驱动）就在设置里再选一次"自动"。
    ...
    // Wrapped in `Rc` so the same context can be handed to the `wire_*`
    // functions instead of each one re-declaring its own field list.
    let ctx = Rc::new(AppContext::new(config, window.clone_strong())?);
    // Only the roots still referenced by `run()` itself are re-exported as
    // locals; after stage A4 the settings seeding and every `wire_*` function
    // take `&ctx` directly.
    ...
    }
    {
        ...
        window.on_open_processes(move || {
            ...
            // 是"渲染时惰性计算"的 —— 不请求重绘就永远没有内容，直到外部原因（用户拖动窗口
            // 边缘）送来一次 Resized 才补上。`request_redraw` 就是 Slint 为此提供的 API。
            // 按 0 / 50 / 200ms 请求三次：首次可能早于窗口映射完成而被 winit 丢弃。
            // 注意：**不是**改尺寸 —— 那只是把用户"拖一下边缘"的动作自动化。
    ...
    }
    {
        ...
        window.on_open_system_info(move || {
            ...
            // 首帧：macOS 上新映射的第二个窗口（同进程窗口）**不会自动产生首次渲染事件**，而 Slint 的布局
            // 是"渲染时惰性计算"的 —— 不请求重绘就永远没有内容，直到外部原因（用户拖动窗口
            // 边缘）送来一次 Resized 才补上。`request_redraw` 就是 Slint 为此提供的 API。
            // 按 0 / 50 / 200ms 请求三次：首次可能早于窗口映射完成而被 winit 丢弃。
            // 注意：**不是**改尺寸 —— 那只是把用户"拖一下边缘"的动作自动化。
    ...
    wire_update_check(&window, &ctx);

    wire_tab_callbacks(&window, &ctx);
    wire_sftp_callbacks(&window, &ctx);
    key_input::wire_key_input(&window, &ctx);
...
///
/// `stage` 决定**同版本号之内**的先后，`num` 是它的序号：
///
/// | stage | 含义 | 例 |
/// |---|---|---|
/// | 0 | 预发布（`-betaN` / `-alphaN` / `-rcN`） | `0.7.9-beta1` **<** `0.7.9` |
/// | 1 | 正式版（无后缀） | `0.7.9` |
/// | 2 | 补丁版（`-fixN`，**已弃用**的旧写法，只为老 tag 保留） | `0.7.9` **<** `0.7.9-fix1` |
///
/// 于是完整的排序是 `0.7.9-beta1 < 0.7.9 < 0.7.9-fix1 < 0.7.10-beta1` ——
...
pub(crate) fn parse_version(s: &str) -> Option<(u32, u32, u32, u8, u32)> {
    ...
    } else if let Some(n) = suffix
        .strip_prefix("beta")
        .or_else(|| suffix.strip_prefix("alpha"))
...
mod version_tests {
    ...
    fn parses_plain_and_prefixed_versions() {
        ...
        // 预发布后缀：stage 0，序号进第 5 位（旧行为是并进 patch，见 #48）。
        assert_eq!(parse_version("0.7.3-rc1"), Some((0, 7, 3, 0, 1)));
        assert_eq!(parse_version("0.7.9-beta2"), Some((0, 7, 9, 0, 2)));
        assert_eq!(parse_version("v0.7.9-alpha3"), Some((0, 7, 9, 0, 3)));
    ...
    /// **beta 必须排在它的基准版之前**（`0.7.9-beta1 < 0.7.9`）—— 这是新的中间版本
    /// 写法：`-betaN` 取代了原来的 `-fixN`；正式版一旦发出去，beta 就不该再被判成"更新"。
    #[test]
    fn beta_releases_sort_before_their_base_version() {
        let v = |s: &str| parse_version(s).unwrap();
        assert!(v("0.7.9-beta1") < v("0.7.9"), "beta 早于正式版");
        assert!(v("0.7.9-beta2") > v("0.7.9-beta1"), "beta 序号递增");
        assert!(v("0.7.9") > v("0.7.9-rc1"), "rc 也算预发布");
        assert!(v("0.7.9-beta9") > v("0.7.8"), "上一个正式版之后才轮到 beta");
        assert!(v("0.7.10-beta1") > v("0.7.9"), "下一个版本的 beta 仍然更新");
        assert!(v("0.7.9-fix1") > v("0.7.9-beta1"), "补丁版仍排在基准版之后");
    }
}
