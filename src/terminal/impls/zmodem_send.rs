//! ZMODEM sender used when the remote shell runs `rz` (#308).
...
mod tests {
    ...
    fn send_blocks_fit_lrzsz_receiver_limit() {
        // Pinned to the exact value, not just "<= 8 KiB": a smaller block only
        // costs throughput, but a larger one overflows lrzsz's receiver buffer
        // and fails as soon as a file reaches one full block (#308).
        assert_eq!(SEND_BLOCK_SIZE, 8 * 1024);
    }
}
