...
/// Build the Quick Connect rows for the given search query. An empty query
/// reproduces the classic grouped layout; a non-empty one flattens matches
/// across saved + built-in sessions with every matching group expanded.
fn build_session_rows(
    ...
    //    de-duplicated, alphabetical. While searching, only groups that hold
    //    at least one match are listed.
    ...
    for (i, s) in builtin_sessions.iter().filter(|s| matches(s)).enumerate() {
