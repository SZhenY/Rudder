...
impl ConfigStore {
    ...
    /// 旧值（`femtovg`（OpenGL）/`auto`/空串）统一升到 `femtovg-wgpu` —— OpenGL 那档已退役。
    #[cfg(target_os = "macos")]
    ...
    /// 矩阵：软件（默认）/ FemtoVG(wgpu→D3D12) / 自动（探测成前两者之一）。
    /// 旧值 `gpu`（OpenGL）与 `skia` 都已退役 → 分别升到 `wgpu` 与软件渲染。
    #[cfg(target_os = "windows")]
    pub fn renderer_mode(&self) -> &str {
        match self.cache.appearance.renderer_mode.as_str() {
            "auto" => "auto",
    ...
    /// Import sessions from a Rudder portable export or a FinalShell connection
    ...
    pub fn import_json(&mut self, raw: &str) -> Result<(usize, usize)> {
        let (sessions, decrypt_rudder_secrets) = match serde_json::from_str::<ExportFile>(raw) {
            Ok(file) => (file.sessions, true),
            Err(rudder_error) => (
                super::finalshell::parse_export(raw).with_context(|| {
                    format!(
                        "not a valid Rudder or FinalShell export file; Rudder parser: {rudder_error}"
                    )
                })?,
                false,
            ),
    ...
    /// Import sessions from a Rudder or FinalShell JSON export file.
...
mod tests {
    ...
    fn renderer_mode_uses_macos_backends_and_validates() {
        let mut store = temp_store();
        // 默认档 = FemtoVG（wgpu → Metal）；OpenGL 那档已从矩阵退役。
        ...
        store.set_renderer_mode("software".into());
        assert_eq!(store.renderer_mode(), "software");
        // 旧配置里的 `femtovg`（OpenGL）升到 wgpu 档，不留在已退役的渲染器上。
